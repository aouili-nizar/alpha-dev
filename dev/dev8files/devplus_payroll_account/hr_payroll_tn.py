...
###################################################################
        ...
        for sal in self.browse(cr, uid, ids):
            ...
            if salaire_net_a_payer :
                ...
                val = {
                    ...
                    'date': date_move,
                    'name': name_move,
                    'amount_currency':salaire_net_a_payer,
    ...
    def _move_create_cotisations_employee(self, cr, uid, ids,bulletin_lines,update):
        ...
        for sal in self.browse(cr, uid, ids):
            ...
            # Journal  and currency
            journal_id = dict_param['account_journal_id'][0]
            param_j=self.pool.get('account.journal')
            id_param_j=param_j.search(cr,uid,[('code','=',"OD")])
            param_od=param_j.browse(cr,uid,id_param_j)
            print"************param_j====================="
            print param_j
            print"************id_param_j============"
            print id_param_j
            ...
            move_c_id1 = self.pool.get('account.move').create(cr, uid, move_c1, context=context)
            self.pool.get('hr.payroll.bulletin').write(cr, uid,bulletin.id, {'move_c_id':move_c_id})
            self.pool.get('hr.payroll.bulletin').write(cr, uid,bulletin.id, {'move_c_id1': move_c_id1})
            ...
            if update:
                ...
                self.pool.get('hr.payroll').write(cr, uid, sal.id, {'move_id': move_c_id1})
            return True
...
class hr_payroll_bulletin_tn(osv.osv):
    ...
    _columns = {
        ...
        'move_c_id1': fields.many2one('account.move', u'Pièce comptable(Declaration cnss)', readonly=True),
